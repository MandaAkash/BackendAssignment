...
app.post('/book-appointment', async(req, res) => {
  try {
    ...
    const existingDate = await Booking.findOne({ selectedDate });
    //If it is first slot
    if(!existingDate){
    ...
    //If it is not first slot
      ...
      if ((existingTimeslot!==null)&&(req.body.selectedTimeSlot===existingTimeslot.selectedTimeSlot)&&(existingDate.selectedDate===req.body.selectedDate)) {
          res.status(200).json({ message: 'The time slot is already filled' });
      } 
      else{
      const dateCount = await Booking.countDocuments({ selectedDate });
